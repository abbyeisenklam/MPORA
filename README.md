...
```bash
docker pull abbyeisenklam/mpora-artifact:v1.0
docker run --gpus all -it abbyeisenklam/mpora-artifact:v1.0
```

<details>
<summary>Building from source</summary>

The profile CSV files are too large for git and are bundled only in the
pre-built image. Extract them from the image first, then build:

```bash
docker create --name mpora-tmp abbyeisenklam/mpora-artifact:v1.0
docker cp mpora-tmp:/artifact/profiles ./profiles
docker rm mpora-tmp

docker build -t mpora-artifact .
docker run --gpus all -it mpora-artifact
```
...
Runtime of 1. and 2. is less than an hour on an Intel(R) Xeon(R) Silver 4216 CPU @ 2.10GHz. 
...
| `inp_feat1`–`inp_feat6` | Benchmark-specific input features (e.g. number of frames, number of vertices, ∈ [0, 1]) |
...
targets (`rem_time` and `next_insn`) from the float XGBoost model.
...
Generated only for Δ = 0.01 (pass `--plot-train-runs` to `train.py`).
